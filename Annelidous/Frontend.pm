...
sub storage {
    my $self=shift;
    return $self->_module_wrapper('_storage_obj', @_)
}
...
sub new_connector {
	...
	my $vm=shift;
	my $storage=shift;
	return $self->connector($self->{'-connector_module'},{-vm=>$vm,-storage=>$storage});
}

sub new_storage {
	my $self=shift;
	my $vm=shift;
	return $self->storage($self->{'-storage_module'},{-vm=>$vm});
}
