...
sub _r_connect {
	$self=shift;
	$self->{_r}=Redis->new( server => 'gtmon.grokthis.net:6379' );
}
...
sub _find {
    ...
    foreach my $client (@cl) {
		...
		$client->{bitness}=$self->r->get(
			join ":", ('client',$client->{id},'pref','cpu_arch')
		);
		$client->{bitness} ||= 64;
		$client->{bitness} = int($client->{bitness});

		$client->{autoheal} = $self->r->get(
			join ":", ('client',$client->{id},'pref','autoheal')
		);
		if (! defined $client->{autoheal} || $client->{autoheal} != 0) {
			$client->{autoheal} = 1;
		}

		$client->{boot_method} = $self->r->get(
			join ":", ('client',$client->{id},'pref','boot_method')
		);
		$client->{boot_method} ||= "pv";
...
sub create {
	...
	my $clientid=shift;
	my $planid=shift;
	my $userid='c'.$clientid.'i'.srvcount($clientid)+1;
	my $salt=join '', ('.', '/', 0..9, 'A'..'Z', 'a'..'z')[rand 64, rand 64];
	my $passplain=String::Random->new();
	my $pass=unix_md5_crypt($passplain, $salt);
	# TODO: make dynamic
	my $server='206.251.37.235';

	# Submit to addpack.php, return serviceid
	# user_login
	# plan_id
	# userid
	# pass
	# server # planned to become depreciated, but still using for now..
$vars=<<VARS;
user_login=$clientid
plan_id=$planid
userid=$userid
...
VARS
	...
	post('https://billing.grokthis.net/addpack.php',$vars);
	
	# Submit to add_device.php
	# dev_desc (req)
	# owner, set to serviceid
	# if fail, run destroy

	# Submit to assign_new_ip.php
	# addr (req)
	# device_id
	# client_id
	# if fail, run destroy
